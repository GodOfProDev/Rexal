...
#include "Rexal/Scene/SceneSerializer.h"

#include "Rexal/Utils/PlatformUtils.h"

#include "ImGuizmo.h"

#include "Rexal/Math/Math.h"
...
# if 0
	...
	void EditorLayer::OnEvent(Event& e)
	{
		...
		dispatcher.Dispatch<KeyPressedEvent>(RX_BIND_EVENT_FN(EditorLayer::OnKeyPressed));
		dispatcher.Dispatch<MouseButtonPressedEvent>(RX_BIND_EVENT_FN(EditorLayer::OnMouseButtonPressed));
	}
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("Rexal Scene (*.rexal)\0*.rexal\0");
	...
	{	
		if (path.extension().string() != ".rexal")
		{
			RX_WARN("Could not load {0} - not a scene file", path.filename().string());
			return;
		}
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("Rexal Scene (*.rexal)\0*.rexal\0");
		if (filepath.has_value())
		{
			SceneSerializer serializer(m_ActiveScene);
